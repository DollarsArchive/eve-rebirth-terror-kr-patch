# EVE rebirth terror 한국어 패치 v1.1.0

**EVE rebirth terror** 비공식 한국어 패치 **v1.1.0**입니다.

이번 버전은 기존 v1.0의 기능, 런타임 수정 및 엔딩 자막을 유지하면서 **2026-08-29 기준 최신 검수 결과와 수동 수정 사항**을 반영했습니다.

## 지원 버전

- Base: `01008BA00F172000` / v0
- Update: `01008BA00F172800` / v131072 (게임 버전 1.0.2)
- RomForge 검증 버전: `1.7.0`
- 패치 방식: `xdelta3 / VCDIFF`

## 패치 내용

이번 배포본의 변경 대상은 총 **94개 파일**입니다.

- 스크립트 BIN 83개
- 시스템 LZS 7개
- 한글 표시용 폰트 3개
- 엔딩 자막 영상 MP4 1개

## 적용 전 확인

압축 파일에 포함된 `설명서.txt`를 먼저 읽어 주세요.

패처는 Base / Update NSP를 검증한 뒤 필요한 파일에만 패치를 적용하여 **모드용 RomFS**를 생성합니다.
원본 NSP를 직접 수정하거나 재패키징하지 않습니다.

본 배포본에는 게임 본편 및 원본 게임 파일, NSP/XCI/NCA, 원본 RomFS, RomForge 본체, `prod.keys`가 포함되어 있지 않습니다.

## 배포 안내

본 패치는 팬 제작 비공식 한국어 패치입니다.

원작 및 관련 콘텐츠의 저작권과 상표권은 각 권리자에게 있습니다.

**본 패치는 게임 원본 파일을 포함하지 않으며, 패치 파일만 제공합니다.**

본 패치의 상업적 판매 및 유료 배포, 원본 게임과 합본 형태의 재배포를 금지합니다.

오역, 오탈자, 미번역, 호칭·말투 문제 또는 패치 적용 오류를 발견하셨다면 저장소의 **Issues**를 통해 제보해 주세요.
